    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.city.update",[$city->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ $city->name }}" required>
                @if($errors->has('name'))
                ...
                <div class="form-group">
                    <label class="required" for="country_id">select country</label>
                    <select name="country_id" id="country_id" required
                        ...
                        @foreach ($country as $item)
                            <option value="{{ $item->id  }}"
                                {{ $item->id == $city->country_id ? 'selected' : null }}>
                                {{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
