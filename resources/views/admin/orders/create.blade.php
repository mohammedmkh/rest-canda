    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.orders.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('resturant') ? 'is-invalid' : '' }}" name="resturant_id" id="resturant_id">
                    @foreach($resturants as $id => $entry)
                        <option value="{{ $id }}" {{ old('resturant_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('delivery_price') ? 'is-invalid' : '' }}" type="number" name="delivery_price" id="delivery_price" value="{{ old('delivery_price', '') }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('receipt_type') ? 'is-invalid' : '' }}" name="receipt_type" id="receipt_type">
                    ...
                    @foreach(App\Models\Order::RECEIPT_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('receipt_type', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
